const Attendance = require('../models/attendanceModel'); // Assuming 'jo' is the correct path to your model

exports.createAttendance = async (req, res) => {
    try {
        // Ensure employeeId is an array
        let { employeeId, ...attendanceData } = req.body;

        if (!Array.isArray(employeeId)) {
            return res.status(400).json({ message: "employeeId must be an array." });
        }

        const newAttendance = new Attendance({
            ...attendanceData,
            employeeId, // Assign employeeId as an array
        });

        await newAttendance.save();

        res.status(201).json({ message: `Attendance Application saved`, data: newAttendance });
    } catch (error) {
